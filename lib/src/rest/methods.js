    ...
    operations.downloadFile = function (context, options, spFilePath, callback) {
        ...
        spr.get(restUrl, { encoding: null })
            .then(function (response) {
                var saveFilePath = path.join(downloadRoot, decodeURIComponent(spFilePath).replace(decodeURIComponent(spFolderBase), ""));

                if (typeof options.omitFolderPath !== "undefined") {
                    var omitFolderPath = path.join(options.omitFolderPath);
                    saveFilePath = path.join(saveFilePath.replace(omitFolderPath, ""));
                }
